/**
 * Generate a Solidity verifier contract directly from the verification key
 * This script uses snarkjs to generate a Solidity verifier contract from the verification key
 */
const fs = require('fs');
...
async function main() {
  console.log('Generating Solidity verifier contract from verification key...');
  
  // Paths
  const verificationKeyPath = path.join(__dirname, '../../backend/zkp/verification_key.json');
  const outputPath = path.join(__dirname, '../contracts/ZKPVerifierFromKey.sol');
  ...
  try {
    ...
    // Create the ABI file
    const contractName = 'Groth16VerifierFromKey';
    const abiJson = {
      contractName,
      sourceName: "contracts/ZKPVerifierFromKey.sol",
      abi: [
        {
          "inputs": [
            {
    ...
    fs.writeFileSync(
      path.join(backendAbisDir, 'Groth16VerifierFromKey.json'),
      JSON.stringify(abiJson, null, 2)
    );
    console.log(`ABI file generated at: ${path.join(backendAbisDir, 'Groth16VerifierFromKey.json')}`);
